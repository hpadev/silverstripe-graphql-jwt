...
class JWTAuthenticator extends MemberAuthenticator
{
    ...
    public function generateToken(HTTPRequest $request, Member $member): Token
    {
        ...
        $token = $builder
            ...
            ->setId($uniqueID) // , true
            ->setHeader('jti', $uniqueID) // can't replicate as a header is now. Need to set it manually
            // Configures the time that the token was issue (iat claim)
            ->setIssuedAt(self::convertToDateTimeImmutable($now))
            // Configures the time that the token can be used (nbf claim)
            ->setNotBefore(self::convertToDateTimeImmutable($now + $config->get('nbf_time')))
            // Configures the expiration time of the token (nbf claim)
            ->setExpiration(self::convertToDateTimeImmutable($now + $config->get('nbf_expiration')))
            // Set renew expiration
            ->set('rexp', self::convertToDateTimeImmutable($now + $config->get('nbf_refresh_expiration')))
    ...
    public static function convertToDateTimeImmutable(int $timestamp): DateTimeImmutable
    {
    ...
    protected function canTokenBeRenewed(Token $parsedToken): bool
    {
        ...
        $dateTimeZone = new DateTimeZone($renewBefore->timezone);
        $renewBeforeDate = new DateTime($renewBefore->date, $dateTimeZone);
        $renewBeforeTimestamp = $renewBeforeDate->getTimestamp();
        $nowTimestamp = DBDatetime::now()->getTimestamp();

        return $renewBeforeTimestamp > $nowTimestamp;
    }
    ...
    {
        $token = 'eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiIsImp0aSI6IjY0ZWQzMzk4YmY2ZTY2LjQwNDMyNzM2In0..svZFhG-xPXikbRxo77plbIFhELNxulkYfyxBAsNkMjY';
        $tokenParsed = $this->parseToken($token);
        d($this->canTokenBeRenewed($tokenParsed));
        d($tokenParsed);
    }
}
