...
class CreateToken
{
    ...
    {
        $createToken = new self();
        $createToken->setJWTAuthenticator(new JWTAuthenticator());

        // Authenticate this member
        $request = Controller::curr()->getRequest();

        $member = $createToken->getAuthenticatedMember($args, $request);

        // Handle unauthenticated
        if (!$member) {
            return $createToken->generateResponse(TokenStatusEnum::STATUS_BAD_LOGIN);
        }

        // Create new token from this member
        $authenticator = $createToken->getJWTAuthenticator();
        $token = $authenticator->generateToken($request, $member);
        return $createToken->generateResponse(TokenStatusEnum::STATUS_OK, $member, $token->__toString());
    }
